...
class StartTransition(PetriNetTransition):
    def __init__(self, action, queue, started):
        PetriNetTransition.__init__(self, 'start')
        self.action_ = action
        self.queue_ = queue
...
class InterruptTransition(PetriNetTransition):
    def __init__(self, action, started, interrupted):
        PetriNetTransition.__init__(self, 'interrupt')
        self.action_ = action
        self.started_ = started
...
class FinishTransition(PetriNetTransition):
    def __init__(self, action, started, interrupted, finished):
        PetriNetTransition.__init__(self, 'finish')
        self.action_ = action
        self.started_ = started
        self.interrupted_ = interrupted
    ...
    def activated(self):
        return (self.action_.IsFinished()
...
class ActionProcess(PetriNet):
    def __init__(self, name, action):
        ...
        self.transitions_.append(
            StartTransition(action, queue, started))
        self.transitions_.append(
            InterruptTransition(action, started, interrupted))
        self.transitions_.append(
            FinishTransition(action, started,interrupted, self.finished_))

        # Put action token in queue.
        queue.AddToken(self.action_.name)

        # Add intention for actions and resources.
        for resource in self.action_.preconditions:
